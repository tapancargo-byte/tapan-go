...
import { InvoicesBulkActions } from "@/features/invoices/invoices-bulk-actions";
...
function InvoicesPageContent() {
	...
	const [_arLoading, setArLoading] = useState(false);

	// Customer create dialog state
	const [customerDialogOpen, setCustomerDialogOpen] = useState(false);
	const [customerDialogTarget, setCustomerDialogTarget] = useState<
		"billing" | "consignor" | "consignee"
	>("billing");
	const [customerDialogLoading, setCustomerDialogLoading] = useState(false);
	const [pendingCustomerResolve, setPendingCustomerResolve] = useState<
		((value: { id: string; name: string } | null) => void) | null
	>(null);
		...
		async function loadInvoices() {
			...
			try {
				...
				Sentry.captureException(error, {
					tags: {
						component: "invoices-page",
						operation: "load-invoices-complete",
					},
				});
				setInvoices([]);
				setLoading(false);
	...
	const handleCustomerDialogSubmit = async (data: {
		...
		try {
			...
			const newCustomer = {
				...
				name: (result as any).name ?? data.name,
			};
			...
			Sentry.captureException(err, {
				tags: {
					component: "invoices-page",
					operation: "quick-create-customer",
				},
			});
			...
			toast({
				...
				description:
					err?.message || "Something went wrong while creating the customer.",
				variant: "destructive",
			});
		} finally {
		...
		try {
			...
			try {
				...
				Sentry.captureException(err, {
					tags: {
						component: "invoices-page",
						operation: "invoice-items-delete",
					},
		...
		try {
			...
			if (shipmentsError) {
				Sentry.captureException(shipmentsError, {
					tags: {
						component: "invoices-page",
						operation: "invoice-shipments-load",
					},
					extra: { invoiceId: invoice.dbId },
				});
				setInvoiceShipments([]);
		...
		try {
			...
			if (insertError) {
				Sentry.captureException(insertError, {
					tags: {
						component: "invoices-page",
						operation: "shipment-invoice-link",
					},
				...
				try {
					...
					if (updateError) {
						Sentry.captureException(updateError, {
							tags: {
								component: "invoices-page",
								operation: "invoice-total-update",
							},
	...
	const handleSubmitInvoice = async (values: InvoiceFormValues) => {
		...
		try {
			...
			const subtotal =
				freight + pickup + packing + delivery + docket + insurance + other;
			...
			if (editingInvoice) {
				...
				const customer =
					customers.find((c) => c.id === data.customer_id) || null;
				...
				setInvoices((prev) =>
					prev.map((inv) => (inv.dbId === updated.dbId ? updated : inv)),
				);
	...
	const getStatusColor = (status: string) => {
		switch (status) {
			...
			case "pending":
				return "bg-warning text-warning-foreground font-medium";
			case "overdue":
				return "bg-destructive text-destructive-foreground font-medium";
			case "partially_paid":
				return "bg-info text-info-foreground font-medium";
			default:
	...
	const renderSmsStatus = (invoiceId: string) => {
		...
		return <span className={`text-xs uppercase mt-0.5 ${color}`}>{label}</span>;
	};
	...
	const handleSelectAll = (selected: boolean) => {
		if (selected) {
			const allIds = filteredInvoices.map((inv) => inv.dbId);
	...
	const handleBulkMarkPaid = async () => {
		...
		const confirm = window.confirm(
			`Mark ${selectedIds.size} invoices as PAID?`,
		);
		if (!confirm) return;
		...
		try {
			...
			setInvoices((prev) =>
				prev.map((inv) =>
					selectedIds.has(inv.dbId) ? { ...inv, status: "paid" } : inv,
				),
			);
			toast({
				title: "Invoices updated",
				description: `${ids.length} invoices marked as PAID.`,
			});
			setSelectedIds(new Set());
		} catch (err: any) {
			toast({
	...
	const handleBulkDelete = async () => {
		if (selectedIds.size === 0) return;
		const confirm = window.confirm(
			`Permanently DELETE ${selectedIds.size} invoices? This cannot be undone.`,
		);
		if (!confirm) return;
		...
		try {
			...
			setInvoices((prev) => prev.filter((inv) => !selectedIds.has(inv.dbId)));
			toast({
				title: "Invoices deleted",
				description: `${ids.length} invoices removed.`,
			});
			setSelectedIds(new Set());
		} catch (err: any) {
			toast({
	...
	const handleBulkDownload = async () => {
		...
		try {
			toast({
				...
				description: `Starting batch generation for ${selectedIds.size} invoices.`,
			});
			...
			for (const id of ids) {
				try {
					...
					// 3. Add to ZIP
					// Find invoice ref for filename
					const invoice = invoices.find((inv) => inv.dbId === id);
					const filename = `Invoice-${invoice?.id || id}.pdf`;
					folder?.file(filename, blob);

					successCount++;
				} catch (err) {
					console.error(`Failed to process invoice ${id}`, err);
					failCount++;
				}
			}

			if (successCount === 0) {
				throw new Error("No invoices could be generated.");
			}

			// 4. Generate and save ZIP
			const content = await zip.generateAsync({ type: "blob" });
			const timestamp = format(new Date(), "yyyy-MM-dd_HH-mm");
			saveAs(content, `Invoices_Batch_${timestamp}.zip`);

			toast({
				title: "Download Ready",
				description: `Successfully zipped ${successCount} invoices.${failCount > 0 ? ` (${failCount} failed)` : ""}`,
			});
			...
			toast({
				...
				description: err.message || "Failed to generate batch download.",
				variant: "destructive",
			});
		} finally {
			setBulkLoading(false);
	...
	const handleCopyTrackingLink = async (shipmentRef: string) => {
		...
		Sentry.addBreadcrumb({
			...
			message: "User attempted to copy tracking link",
			level: "info",
			data: { shipmentRef: trimmedRef },
		});
		...
		if (!origin) {
			toast({
				...
				description:
					"Tracking link could not be generated in this environment.",
				variant: "destructive",
			});
			return;
		}
				...
				<div className="flex gap-4 flex-col sm:flex-row items-start sm:items-end">
					...
					{roleLoaded && !canEdit && (
						<p className="text-xs text-muted-foreground max-w-xs">
							You have read-only billing access. Contact an admin to create
							invoices.
						</p>
					)}
