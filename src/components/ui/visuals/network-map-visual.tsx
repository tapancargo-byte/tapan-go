	...
	return (
		<div className="relative flex h-full w-full items-center justify-center overflow-hidden rounded-xl border border-border/60 bg-pop/80 dark:bg-background/80 p-4">
			...
			<div className="absolute inset-0 flex items-center justify-center opacity-20">
				<div className="h-[200px] w-[200px] rounded-full border border-chart-2/30 animate-[ping_4s_ease-out_infinite]" />
				<div className="absolute h-[140px] w-[140px] rounded-full border border-chart-2/30 animate-[ping_4s_ease-out_infinite_1s]" />
			</div>
			...
			>
				...
				{nodes.slice(1).map((node, i) => (
					<circle key={`moving-${i}`} r={4} fill="var(--chart-2)" opacity={0.9}>
						<animateMotion
							dur="3s"
				...
				{nodes.map((node, i) => (
					<g key={i} transform={`translate(${node.x}, ${node.y})`}>
						<motion.circle
							r={node.r}
							fill={i === 0 ? "var(--chart-2)" : "var(--muted-foreground)"}
							stroke={i === 0 ? "var(--chart-2)" : "var(--border)"}
							strokeWidth="2"
							initial={{ scale: 0 }}
							animate={{ scale: 1 }}
