...
async def run_test():
    ...
    try:
        ...
        # -> Input email and password for billing staff and click login button
        ...
        await page.wait_for_timeout(3000); await elem.fill('billingpassword')
        ...
        # -> Retry login with correct credentials or report issue
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        # Input correct billing staff password
        elem = frame.locator('xpath=html/body/div/main/div/section[2]/div/form/div[2]/input').nth(0)
        ...
        # -> Check if there is an option to recover password or retry login with different credentials
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Invoice Fully Paid').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: Partial payment on invoice did not update invoice status to 'partial payment' and audit log entry was not created as expected.")
