...
import { type CustomerFormValues, customerSchema } from "@/lib/validations";
...
export default function CustomersPage() {
	...
	const [editingCustomer, setEditingCustomer] = useState<UICustomer | null>(
		null,
	);
	...
	useEffect(() => {
		...
		async function loadCustomers() {
			...
			try {
				...
				const baseCustomers: UICustomer[] = ((data as any[]) ?? []).map(
					(row) => ({
						dbId: row.id,
						id: row.id,
						name: row.name ?? "",
					...
					try {
						...
						if (invoicesError) {
							...
							((invoiceRows as any[]) ?? []).forEach((row) => {
								...
								const current = billingByCustomer.get(cid) ?? {
									totalAmount: 0,
									outstandingAmount: 0,
								...
								if (invoiceDate) {
									if (
										!next.lastInvoiceDate ||
										invoiceDate > next.lastInvoiceDate
									) {
