...
async def run_test():
    ...
    try:
        ...
        # -> Check for alternative login options or verify credentials.
        frame = context.pages[-1]
        # Click 'Forgot password?' to check alternative login options or recovery.
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Manifest scanning session completed successfully').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test failed: Manifest scanning session finalization did not aggregate scanned barcodes or update manifest records with origin, destination, and airline data as expected.')
