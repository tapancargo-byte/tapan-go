...
function getFreepikApiKey(): string {
	...
	if (!key) {
		throw new Error(
			"FREEPIK_API_KEY is not set. Add it to your environment or .env.local.",
		);
	}
	return key;
}

async function freepikRequest<T = any>(
	path: string,
	init: RequestInit = {},
): Promise<T> {
...
export async function downloadIconById(
	id: string,
	options: { format?: string; size?: string } = {},
) {
...
export async function renderGeneratedIcon(
	taskId: string,
	format: "png" | "svg" = "png",
) {
	// POST /v1/ai/text-to-icon/{task-id}/render/{format}
	return freepikRequest(
		`/ai/text-to-icon/${encodeURIComponent(taskId)}/render/${format}`,
		{
			method: "POST",
		},
	);
