...
import { LOCATION_SCOPES, LOCATIONS } from "@/types/auth";
...
export function LocationProvider({
	...
	const [locationScope, setLocationScopeState] =
		useState<LocationScope>(defaultLocation);
	const [userHomeLocation, setUserHomeLocationState] =
	...
	const setLocationScope = useCallback((scope: LocationScope) => {
		...
		window.dispatchEvent(
			new CustomEvent("location-scope-changed", { detail: scope }),
		);
	}, []);
	...
	const scopeLabel =
		LOCATION_SCOPES.find((s) => s.value === locationScope)?.label || "Unknown";
...
export function useLocationScopeChange(
	callback: (scope: LocationScope) => void,
) {
	useEffect(() => {
