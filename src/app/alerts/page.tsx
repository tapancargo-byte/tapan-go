	...
	useEffect(() => {
		...
		async function loadAlerts() {
			...
			try {
				...
				const invoiceRows = (
					[...(overdueRes.data ?? []), ...(pendingRes.data ?? [])] as any[]
				).filter(Boolean);
	...
	const handleSyncToNotifications = async () => {
		...
		if (total === 0) {
			toast({
				...
				description:
					"There are currently no alerts to push into notifications.",
			});
			return;
		}
	...
	return (
		<DashboardPageLayout
			header={{
				...
				description:
					"Track overdue invoices, stalled shipments, and escalations",
				icon: BracketsIcon,
			}}
		>
