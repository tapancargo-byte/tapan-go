...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        # Input alternative operator email
        elem = frame.locator('xpath=html/body/div/main/div/section[2]/div/form/div/input').nth(0)
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Offline Scan Queue Synchronization Complete').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test failed: Offline scan queue did not persist scan data locally or failed to synchronize automatically upon network restoration as per the test plan.")
