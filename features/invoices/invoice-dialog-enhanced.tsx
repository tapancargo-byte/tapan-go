...
export function InvoiceDialogEnhanced({
  ...
  const [rateLookupStatus, setRateLookupStatus] = React.useState<"idle" | "found" | "not_found">("idle");
  ...
  const [refMode, setRefMode] = React.useState<"auto" | "manual">(isCreateMode ? "auto" : "manual");
  const [previewRef, setPreviewRef] = React.useState<string>("");

  // Track if we've manually set a value (to prevent overwriting scanned values)
  const [hasScannedValue, setHasScannedValue] = React.useState(false);

  React.useEffect(() => {
    // Only run when dialog is open
    if (!open) return;

    if (refMode === "auto" && isCreateMode) {
      supabase.rpc("preview_next_invoice_ref")
        .then(({ data, error }) => {
          if (error) {
            console.error("Failed to fetch invoice ref:", error);
            form.setValue("invoiceRef", "ERR-" + Date.now()); // Fallback
            return;
          }
          if (data) {
            setPreviewRef(data);
            form.setValue("invoiceRef", data);
            setHasScannedValue(false);
          }
        });
    } else if (refMode === "manual" && isCreateMode && !hasScannedValue) {
      const currentValue = form.getValues("invoiceRef");
      if (!currentValue || currentValue === previewRef) {
        form.setValue("invoiceRef", "");
      }
    }
  }, [open, refMode, isCreateMode, form, previewRef, hasScannedValue]);
  ...
  const handleBarcodeScan = React.useCallback((code: string) => {
    ...
    toast({
      ...
      description: `Consignment No. set to ${code}`,
    });
  }, [form, toast]);

  const scannerEnabled = open && isCreateMode && refMode === "manual";
  // Debug log removed to reduce re-render noise

  useBarcodeScanner({
    enabled: scannerEnabled,
    onScan: handleBarcodeScan,
    debug: false, // Disabled to reduce timing issues
  });

  // Watch for rate lookup triggers and freight calculation
  ...
  React.useEffect(() => {
    ...
    const citiesMatch = (input: string, rateCity: string): boolean => {
      ...
      const rateCityWords = rateCityLower.split(/[\s,.-]+/).filter(w => w.length > 1);

      // Direct inclusion check
      if (rateCityLower.includes(input) || input.includes(rateCityLower)) return true;

      // Word-based matching (e.g., "New Delhi" matches "Delhi")
      for (const word of inputWords) {
        if (rateCityLower.includes(word) || rateCityWords.some(rw => rw.includes(word) || word.includes(rw))) {
          return true;
        }
      }
      return false;
    ...
    const foundFallbackDirect = !foundDirect && !foundReverse
      ? rates.find((r) => {
        const rOrigin = r.origin.toLowerCase().trim();
        const rDest = r.destination.toLowerCase().trim();
    ...
    const foundFallbackReverse = !foundDirect && !foundReverse && !foundFallbackDirect
      ? rates.find((r) => {
        const rOrigin = r.origin.toLowerCase().trim();
        const rDest = r.destination.toLowerCase().trim();
    ...
    const matchedRateResult =
      foundDirect || foundReverse || foundFallbackDirect || foundFallbackReverse;
  ...
  const calculatedTotals = React.useMemo(() => {
    ...
    const subtotal = freight + pickup + packing + docket + delivery + insurance + other;
  ...
  const onInvalid = React.useCallback(
    (errors: Record<string, any>) => {
      ...
      const findFirstFieldName = (obj: any, path: string[] = []): string | null => {
        if (!obj || typeof obj !== "object") return null;
        for (const key of Object.keys(obj)) {
          const val = obj[key];
          if (val && typeof val === "object") {
            // react-hook-form field error typically has { message, type, ref }
            if (typeof val.message === "string") return [...path, key].join(".");
      ...
      const firstName = findFirstFieldName(errors) || Object.keys(errors ?? {})[0] || null;
                  ...
                  <div className="col-span-1 md:col-span-1">
                    ...
                    <div className="flex gap-2">
                      {isCreateMode && (
                        <Select
                          value={refMode}
                          onValueChange={(val: "auto" | "manual") => setRefMode(val)}
                        >
                            ...
                            <FormItem>
                              <FormControl>
                                <div className="relative">
                                  <Input
                                    placeholder={refMode === "auto" ? "Loading..." : "Scan or Type..."}
                                    readOnly={refMode === "auto" || !isCreateMode} // Allow edit only if creating + manual
                                    className={cn(
                                      "h-9 font-mono",
                                      refMode === "auto" && "bg-muted/40 text-muted-foreground",
                                      refMode === "manual" && "pl-8 border-primary"
                                    )}
                                    {...field}
                                    onKeyDown={(e) => {
                                      // Prevent form submission on Enter when scanning/typing manually
                                      if (refMode === "manual" && e.key === "Enter") {
                                        e.preventDefault();
                                      }
                                    }}
                                  />
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              <AddNewButton
                                ...
                                onClick={async () => {
                                  ...
                                  if (created) {
                                    form.setValue("consignorId", created.id, { shouldDirty: true });
                                    form.setValue("consignorName", created.name, { shouldDirty: true });
                                  }
                                }}
                              />
                            </div>
                            <FormControl>
                              <Select
                                ...
                                onValueChange={(val) => {
                                  ...
                                  if (customer) {
                                    form.setValue("consignorName", customer.name, { shouldDirty: true });
                                  }
                                }}
                              >
                                <SelectTrigger className="h-9">
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Input placeholder="MR JOHNSON" className="h-9" {...field} />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Input placeholder="SAFDARJUNG NEW DELHI - 110029" className="h-9" {...field} />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Input placeholder="9873530487" className="h-9" {...field} />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              <AddNewButton
                                ...
                                onClick={async () => {
                                  ...
                                  if (created) {
                                    form.setValue("consigneeId", created.id, { shouldDirty: true });
                                    form.setValue("consigneeName", created.name, { shouldDirty: true });
                                  }
                                }}
                              />
                            </div>
                            <FormControl>
                              <Select
                                ...
                                onValueChange={(val) => {
                                  ...
                                  const customer = customers.find(c => c.id === val);
                                  if (customer) {
                                    form.setValue("consigneeName", customer.name, { shouldDirty: true });
                                  }
                                }}
                              >
                                <SelectTrigger className="h-9">
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Input placeholder="MISS DIANA" className="h-9" {...field} />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Input placeholder="SINGJAMEI IMF - 795001" className="h-9" {...field} />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Input placeholder="9863428811" className="h-9" {...field} />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                ...
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="text-sm font-semibold text-foreground">Courier Details & Rate</h3>
                    </div>
                    {/* Rate Lookup Status */}
                    {rateLookupStatus === "found" && matchedRate && (
                      <Badge variant="default" className="bg-green-500/10 text-green-600 border-green-200 gap-1">
                        ...
                        Rate found: ₹{matchedRate.ratePerKg}/kg + ₹{matchedRate.baseFee} base
                      </Badge>
                    )}
                    {rateLookupStatus === "not_found" && origin && destination && (
                  ...
                  <div className="mb-4">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-xs font-medium mb-2 block">Transport Mode</FormLabel>
                          <div className="flex gap-2">
                            <Button
                              type="button"
                              variant={field.value === "air" ? "default" : "outline"}
                              size="sm"
                              className={cn(
                                "flex-1 gap-2 h-10",
                                field.value === "air" && "bg-blue-600 hover:bg-blue-700"
                              )}
                            ...
                            <Button
                              type="button"
                              variant={field.value === "express" ? "default" : "outline"}
                              size="sm"
                              className={cn(
                                "flex-1 gap-2 h-10",
                                field.value === "express" && "bg-amber-600 hover:bg-amber-700"
                              )}
                            ...
                            <Button
                              type="button"
                              variant={field.value === "surface" ? "default" : "outline"}
                              size="sm"
                              className={cn(
                                "flex-1 gap-2 h-10",
                                field.value === "surface" && "bg-emerald-600 hover:bg-emerald-700"
                              )}
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-xs font-medium">Origin</FormLabel>
                          <FormControl>
                            <Input placeholder="Delhi" className="h-9" {...field} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-xs font-medium">Destination</FormLabel>
                          <FormControl>
                            <Input placeholder="Manipur" className="h-9" {...field} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-xs font-medium">No. of Pieces</FormLabel>
                          <FormControl>
                            <Input
                              type="number"
                              min="1"
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-xs font-medium">Actual Weight (Kg)</FormLabel>
                          <FormControl>
                            <Input
                              type="number"
                              min="0"
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-xs font-medium">Charged Weight (Kg)</FormLabel>
                          <FormControl>
                            <Input
                              type="number"
                              min="0"
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-xs font-medium">Payment Mode</FormLabel>
                          <FormControl>
                            <Select
                              value={field.value ?? ""}
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel className="text-xs font-medium">Freight (₹)</FormLabel>
                            <FormControl>
                              <Input
                                type="number"
                                min="0"
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel className="text-xs font-medium">Pickup Charge (₹)</FormLabel>
                            <FormControl>
                              <Input
                                type="number"
                                min="0"
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel className="text-xs font-medium">Packing (₹)</FormLabel>
                            <FormControl>
                              <Input
                                type="number"
                                min="0"
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel className="text-xs font-medium">Docket Charges (₹)</FormLabel>
                            <FormControl>
                              <Input
                                type="number"
                                min="0"
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel className="text-xs font-medium">Delivery (₹)</FormLabel>
                            <FormControl>
                              <Input
                                type="number"
                                min="0"
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel className="text-xs font-medium">Insurance Charge (₹)</FormLabel>
                            <FormControl>
                              <Input
                                type="number"
                                min="0"
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel className="text-xs font-medium">Other Charges (₹)</FormLabel>
                            <FormControl>
                              <Input
                                type="number"
                                min="0"
                    ...
                    <div className="bg-muted/50 rounded-lg p-4 space-y-2 lg:sticky lg:top-4 h-fit">
                      ...
                      <div className="space-y-1.5 text-sm">
                        <div className="flex justify-between">
                          ...
                          <span>₹{calculatedTotals.freight.toLocaleString("en-IN")}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Pickup Charge</span>
                          <span>₹{calculatedTotals.pickup.toLocaleString("en-IN")}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Packing</span>
                          <span>₹{calculatedTotals.packing.toLocaleString("en-IN")}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Docket Charges</span>
                          <span>₹{calculatedTotals.docket.toLocaleString("en-IN")}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Insurance Charge</span>
                          <span>₹{calculatedTotals.insurance.toLocaleString("en-IN")}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">GST ({calculatedTotals.gstPercent}%)</span>
                          <span>₹{calculatedTotals.gstAmount.toLocaleString("en-IN")}</span>
                        </div>
                        {calculatedTotals.other > 0 && (
                          <div className="flex justify-between">
                            <span className="text-muted-foreground">Other</span>
                            <span>₹{calculatedTotals.other.toLocaleString("en-IN")}</span>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex justify-between text-sm text-green-600">
                        ...
                        <span>₹{calculatedTotals.advance.toLocaleString("en-IN")}</span>
                      </div>
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span
                          className={cn(
                            "font-bold text-xl",
                            calculatedTotals.balanceDue > 0 ? "text-orange-600" : "text-green-600"
                          )}
                        >
