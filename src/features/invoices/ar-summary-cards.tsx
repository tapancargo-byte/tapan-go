...
function MetricCard({
	...
	const variants = {
		...
		success: "border-l-4 border-l-chart-2 bg-chart-2/5",
		warning: "border-l-4 border-l-chart-3 bg-chart-3/5",
	...
	return (
		<Card
			className={cn(
				"border-t-0 border-r-0 border-b-0 shadow-lg",
				variants[variant],
			)}
		>
...
export function ArSummaryCards({ arSummary }: ArSummaryCardsProps) {
	...
	const agingChartData = aging
		? [
				...
				{
					bucket: "days61plus",
					amount: aging.days61plus.amount,
					count: aging.days61plus.count,
					fill: "var(--color-days61plus)",
				},
			]
		: [];
		...
		<div className="space-y-6">
			...
			<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
				...
				<MetricCard
					...
					variant="success"
					description={
						arSummary.collectionsMomentum !== undefined
							? `${arSummary.collectionsMomentum > 0 ? "+" : ""}${arSummary.collectionsMomentum}% vs last 7d`
							: `${collectionRate}% collection rate`
					}
					trend={
						arSummary.collectionsMomentum && arSummary.collectionsMomentum < 0
							? "down"
							: "up"
					}
				/>
				...
				<div className="grid gap-4 md:grid-cols-2">
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Invoice counts and amounts by age
							</CardDescription>
						</CardHeader>
							...
							<div className="flex items-center justify-between p-3 rounded-lg border border-border bg-card">
								<div className="flex items-center gap-3">
									...
									<div>
										...
										<div className="text-sm text-muted-foreground">
											{aging.days1to30.count} invoices
										</div>
									</div>
								</div>
								<div className="text-xl font-bold text-foreground">
							...
							<div className="flex items-center justify-between p-3 rounded-lg border border-border bg-card">
								<div className="flex items-center gap-3">
									...
									<div>
										...
										<div className="text-sm text-muted-foreground">
											{aging.days31to60.count} invoices
										</div>
									</div>
								</div>
								<div className="text-xl font-bold text-foreground">
							...
							<div className="flex items-center justify-between p-3 rounded-lg border border-border bg-card">
								<div className="flex items-center gap-3">
									...
									<div>
										...
										<div className="text-sm text-muted-foreground">
											{aging.days61plus.count} invoices
										</div>
									</div>
								</div>
								<div className="text-xl font-bold text-foreground">
