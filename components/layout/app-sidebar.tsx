...
import { ChevronLeft, ChevronRight, MapPin, Bell, ChevronDown } from "lucide-react";
        ...
        <PopoverTrigger asChild>
          <button className="flex-1 flex items-center gap-2 px-3 py-2 rounded-md bg-muted/50 hover:bg-muted transition-colors text-left">
            ...
            <span className="text-xs font-medium truncate flex-1">
              {locationScope === "all" ? "All Locations" : LOCATIONS[locationScope as Location]?.name || "Imphal"}
            </span>
...
export function AppSidebar() {
  ...
  const [sidebarCounts, setSidebarCounts] = React.useState<Record<NavBadgeKey, number | null>>({
    warehouses: null,
    shipments: null,
    invoices: null,
    alerts: null,
      ...
      <div className="flex h-full flex-col">
        ...
        <div className="flex h-16 items-center border-b px-3 gap-2">
          <div className="flex items-center gap-2 flex-1">
            <TacLogo collapsed={isCollapsed} className={cn("", isCollapsed && "pl-1")} />
          </div>
