...
export default function BarcodeManagementPage() {
		...
		async function loadBarcodes() {
			...
			try {
				...
				const shipmentIds = Array.from(
					new Set(
						rows.map((b) => b.shipment_id).filter((id): id is string => !!id),
					),
				);
				...
				const normalized: UIBarcode[] = rows.map((b) => {
					const shipment = b.shipment_id
						? shipmentsMap.get(b.shipment_id)
						: undefined;
	...
	const handleScanResult = async (scannedData: string) => {
		...
		if (found) {
			...
			const filtered = barcodes.filter(
				(bc) => bc.barcodeNumber === scannedData,
			);
			setFilteredBarcodes(filtered);
	...
	const headerConfig = {
		...
		description:
			"Scan and manage shipment barcodes across your logistics network",
		icon: BracketsIcon,
	};
										...
										filteredBarcodes.map((barcode) => (
											...
											>
												...
												<td className="py-3 px-4 text-gray-600">
													{barcode.scannedAt
														? new Date(barcode.scannedAt).toLocaleDateString()
														: ""}
												</td>
