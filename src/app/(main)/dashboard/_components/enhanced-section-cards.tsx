...
function MiniRadialChart({ value, color }: { value: number; color: string }) {
...
function EnhancedStatCard({
	...
	return (
		<Card className="@container/card bg-primary/5 shadow-xs overflow-hidden transition-all hover:shadow-md">
			...
			<CardContent className="flex items-center justify-between pb-4">
				...
				<MiniRadialChart value={chartValue} color={chartColorClass} />
			</CardContent>
		</Card>
	);
}
...
function SimpleStatCard({
	...
	return (
		<Card className="@container/card bg-primary/5 shadow-xs transition-all hover:shadow-md">
			<CardHeader className="pb-2">
				<CardDescription className="flex items-center gap-2 text-xs">
					{icon}
					{title}
...
export function EnhancedSectionCards({ stats }: EnhancedSectionCardsProps) {
	...
	const _invoicesProgress = Math.min((data.pendingInvoices / 50) * 100, 100);
