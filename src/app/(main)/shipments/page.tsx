...
const ETAUpdateDialog = dynamic(
	() =>
		import("@/components/shipments/eta-update-dialog").then((mod) => ({
			default: mod.ETAUpdateDialog,
		})),
	{ ssr: false },
);
...
function ShipmentsTrackingContent() {
	...
	const [selectedShipment, setSelectedShipment] = useState<UIShipment | null>(
		null,
	);
	...
	useEffect(() => {
		...
		async function loadShipments() {
			...
			try {
				...
				const shipmentRows: ShipmentRecord[] =
					(data as ShipmentRecord[] | null) ?? [];
				...
				try {
					...
					if (customersError) {
						Sentry.captureException(customersError, {
							tags: {
								component: "shipments-page",
								operation: "customers-load",
							},
						});
					} else {
				...
				const normalized: UIShipment[] = shipmentRows.map((s) => {
					const customer = s.customer_id
						? customersMap.get(s.customer_id)
						: undefined;
	...
	const handleSubmitShipment = async (values: ShipmentFormValues) => {
		...
		try {
			...
			const basePayload: any = {
				...
				customer_id:
					customerId && customerId !== "__unassigned__" ? customerId : null,
	...
	const loadBarcodesForShipment = async (shipment: UIShipment) => {
		...
		try {
			...
			const normalized: ShipmentBarcode[] = ((data as any[]) ?? []).map(
				(row) => ({
					id: row.id,
		...
		try {
			...
			if (barcodesError) {
				Sentry.captureException(barcodesError, {
					tags: {
						component: "shipments-page",
						operation: "timeline-barcodes-load",
					},
					extra: { shipmentId: shipment.dbId },
				});
				setShipmentTimeline([]);
			...
			const events: ShipmentScanEvent[] = ((scans as any[]) ?? []).map(
				(row) => ({
					id: row.id as string,
					barcodeNumber:
	...
	const handleOpenEtaDialog = async (shipment: UIShipment) => {
		...
		const { data, error } = await supabase
			.from("shipments")
			.select(
				"id, shipment_ref, origin, destination, etd, atd, eta, ata, carrier_name, awb_number, transport_mode, eta_notes",
			)
			.eq("id", shipment.dbId)
			.single();
														...
														<div className="text-right">
															...
															{bc.lastScannedAt && (
																<p className="text-xs text-muted-foreground mt-0.5">
																	{new Date(bc.lastScannedAt).toLocaleString(
																		"en-IN",
																	)}
																</p>
															)}
