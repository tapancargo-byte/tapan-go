...
const chatStore = create<ChatStore>((set, get) => ({
	...
	logSystemMessage: (conversationId, content) => {
		...
		let found = false;
		const updatedConversations: ChatConversation[] = conversations.map(
			(conv) => {
		...
		(async () => {
			try {
				...
				const conversations: ChatConversation[] = ticketRows.map((ticket) => {
					...
					const mappedMessages: ChatMessage[] = ticketMessages.map(
						(m: any) => ({
							id: String(m.id),
							content: String(m.content ?? ""),
							timestamp: (m.created_at as string) ?? new Date().toISOString(),
							senderId:
								m.sender_role === "operator" ? CURRENT_USER.id : "customer",
