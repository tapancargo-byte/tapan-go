...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Invoice Download Successful').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test failed: Signed URLs for downloading invoices and barcodes did not expire as expected, allowing unauthorized access.')
