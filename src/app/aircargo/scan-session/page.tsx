	...
	const handleScan = async (barcodeValue: string) => {
        ...
        `)
          ...
          `)
				...
				throw new Error(
					`Barcode ${trimmed} not found in system (checked cargo barcodes and invoices).`,
				);
			}
	...
	const handleCreateManifest = async () => {
		...
		if (!canSubmit) {
			toast({
				...
				description:
					"Please fill in Origin Hub, Destination, and Airline Code first.",
				variant: "destructive",
			});
			return;
		}
				...
				<Card>
					...
					<CardContent className="space-y-3">
						...
						<div className="text-xs text-muted-foreground">
							All fields are required to create a manifest from this scan
							session.
						</div>
					</CardContent>
				</Card>
