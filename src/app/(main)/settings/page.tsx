...
export default function SettingsPage() {
	...
	const handleAvatarChange = async (
		...
		try {
			...
			if (updateError) {
				console.error(
					"Failed to update avatar_url on users",
					updateError.message,
				);
				toast({
					...
					description:
						updateError.message ||
						"Something went wrong while saving your avatar.",
					variant: "destructive",
				});
				return;
			}
	...
	const handleSaveOrgProfile = async () => {
		if (!canEditOrg) {
			toast({
				...
				description:
					"Only manager or admin users can update organization settings.",
				variant: "destructive",
			});
			return;
		}
		...
		try {
			...
			toast({
				...
				description:
					err?.message ||
					"Something went wrong while saving organization settings.",
				variant: "destructive",
			});
		} finally {
			setOrgSaving(false);
	...
	return (
		<DashboardLayout
			header={{
				title: "Settings",
				description:
					"Manage system configuration, preferences, and user settings",
				icon: GearIcon,
			}}
		>
							...
							<CardContent className="space-y-4">
								<div className="space-y-2">
									...
									<p className="text-xs text-muted-foreground">
										Recommended square image, up to a few MB. This will show in
										the sidebar.
									</p>
								</div>
							</CardContent>
							...
							<CardContent className="space-y-6">
								...
								<div className="flex items-center justify-between gap-3">
									{roleLoaded && !canEditOrg && (
										<p className="text-xs text-muted-foreground">
											You have read-only access. Contact an admin to update
							...
							<CardContent className="space-y-4">
								<div className="flex items-center justify-between">
									<div>
										...
										<p className="text-sm text-muted-foreground">
											Receive alerts for shipment status changes
										</p>
									</div>
									<Switch defaultChecked />
								</div>
								<div className="flex items-center justify-between">
									<div>
										...
										<p className="text-sm text-muted-foreground">
											Get notified about warehouse events
										</p>
									</div>
									<Switch defaultChecked />
								</div>
								<div className="flex items-center justify-between">
									<div>
										...
										<p className="text-sm text-muted-foreground">
											Receive invoice payment reminders
										</p>
									</div>
									<Switch defaultChecked />
								</div>
						...
						<Card>
							...
							<CardContent className="space-y-4">
								...
								<div className="pt-4 border-t">
									...
									{qrCode && (
										<div className="space-y-3 rounded-md border bg-muted/40 p-3">
											<p className="text-sm font-medium">
												Scan the QR code and enter the 6-digit code to finish
												setup.
											</p>
									...
									{hasTotp && !qrCode && (
										<p className="text-xs text-muted-foreground">
											Two-factor authentication is active. Admin and manager
											accounts will be required to complete an extra step when
											signing in.
										</p>
									)}
								</div>
