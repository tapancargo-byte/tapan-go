...
import { type RateFormValues, rateSchema } from "@/lib/validations";
	...
	const handleSubmitRate = async (values: RateFormValues) => {
		...
		try {
			...
			toast({
				...
				description:
					err?.message || "Something went wrong while saving the rate.",
				variant: "destructive",
			});
		} finally {
			setIsCreating(false);
