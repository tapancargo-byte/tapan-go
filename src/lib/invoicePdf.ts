...
import { bankDetails, companyProfile, paymentConfig } from "./companyConfig";
import { sendInvoiceFailureAlert } from "./slackAlerts";
import { createSignedUrl, uploadBufferToStorage } from "./storageHelpers";
	...
	try {
		...
		try {
			...
			if (process.env.NODE_ENV !== "production") {
				try {
					...
					console.warn(
						"Failed to capture invoice debug screenshot",
						screenshotError,
					);
				}
			}
		...
		try {
			...
			if (!logsError && recentLogs && recentLogs.length >= 3) {
				const allFailed = recentLogs.every(
			...
			console.error(
				"Failed to process Slack alert for invoice generation failure",
				alertError,
			);
		}
...
function buildInvoiceHtml({
  ...
  `;
	...
	const getStatusStyle = (s: string) => {
		switch (s) {
			case "PAID":
				return { bg: PDF_COLORS.success, text: "#fff" };
			case "OVERDUE":
				return { bg: "#dc2626", text: "#fff" };
			default:
				return { bg: PDF_COLORS.brand, text: "#fff" };
		}
	};
