...
const statusBadgeClasses = (status?: string | null) => {
	...
	if (s === "unpaid") return "bg-warning/10 text-warning border-warning/30";
		...
		<main className="min-h-screen bg-background text-foreground flex items-center justify-center px-4 py-8">
			<div className="w-full max-w-2xl space-y-8">
				...
				<div className="flex flex-col items-center gap-6">
					<UnifiedLogo className="scale-150 origin-center mb-2" />
									...
									{!hasShipments && (
										<div className="mt-4 p-3 bg-muted/30 rounded-md">
											<p className="text-xs text-muted-foreground">
												No shipments are currently linked to this invoice.
												Shipment tracking will be available once packages are
												dispatched.
											</p>
										</div>
									)}
										...
										{result.shipment.transport_mode && (
											<div>
												...
												<div className="font-medium capitalize">
													{result.shipment.transport_mode === "air"
														? "✈ Air Cargo"
														: result.shipment.transport_mode}
												</div>
											</div>
											...
											<div className="grid gap-2 text-sm sm:grid-cols-2">
												{result.shipment.etd && (
													<div>
														...
														<div className="font-medium">
															{formatDateTime(
																result.shipment.atd || result.shipment.etd,
															)}
														</div>
													</div>
												)}
												{(result.shipment.eta || result.shipment.ata) && (
													<div>
														...
														<div
															className={`font-medium ${result.shipment.ata ? "text-success" : "text-brand"}`}
														>
															{formatDateTime(
																result.shipment.ata || result.shipment.eta,
															)}
				...
				<p className="text-xs text-center text-muted-foreground">
					Data is updated as your packages are scanned at different locations.
					If you think there is an issue with your shipment, please contact
					customer support.
				</p>
