...
export default function AircargoPage() {
	...
	const handleAddShipmentToManifest = async () => {
		...
		try {
			...
			setManifests((prev) =>
				prev.map((m) =>
					m.id === activeManifest.id ? { ...m, shipments: m.shipments + 1 } : m,
				),
			);
			...
			<div className="flex flex-col gap-6">
										...
										<FormField
											...
											render={({ field }) => (
												<FormItem>
													...
													<FormControl>
														<Input placeholder="6E, SG, G8, ..." {...field} />
													</FormControl>
													<FormMessage />
												</FormItem>
											)}
				...
				<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
					{filteredManifests.map((manifest) => (
						<Card
							key={manifest.id}
							className="p-6 border-border bg-card hover:bg-accent/5 hover:border-primary/20 transition-all duration-300 group"
						>
							<div className="flex justify-between items-start mb-4">
								...
								<Badge
									variant="outline"
									className={getStatusColor(manifest.status)}
								>
