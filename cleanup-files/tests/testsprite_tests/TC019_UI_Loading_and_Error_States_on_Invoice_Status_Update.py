...
async def run_test():
    ...
    try:
        ...
        # -> Retry login with correct credentials or verify credentials to proceed.
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Invoice update successful').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: Invoice status updates did not show loading indicators, handle error states gracefully, or use optimistic UI updates as required by the test plan.")
