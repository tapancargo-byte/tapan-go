...
export async function POST(req: Request) {
	try {
		...
		const { data, error } = await supabaseAdmin
			.from("invoices")
			.select(
				"id, invoice_ref, amount, customer_id, customers:customer_id ( id, name, phone )",
			)
			.eq("id", invoiceId)
			.maybeSingle();
		...
		if (!to.startsWith("+")) {
			...
			if (digitsOnly.length === 10) {
				if (defaultCountryCode) {
					...
					console.warn(
						"10-digit phone number requires WHATSAPP_DEFAULT_COUNTRY_CODE env var",
					);
					return NextResponse.json(
						{
							error:
								"Phone number requires country code. Please update to include country code (e.g., +91XXXXXXXXXX)",
						},
						{ status: 400 },
					);
				}
		...
		const metaError =
			(waJson?.error as Record<string, unknown> | undefined) ?? undefined;
